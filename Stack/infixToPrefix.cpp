...
public:
   ...
   string infixToPostfix(string infix);//function for converting infix argument to postfix.
   string infixToPrefix(string infix);//function for converting infix argument to prefix.
};
...
string Stack::infixToPrefix(string infix){
    ...
    prefix=infixToPostfix(infix);//((a / b) + c) - (d + (e * f ))
    l=prefix.length();
    for(int i=0; i<l/2; i++){
        temp=prefix[i];
...
int main()
{
    ...
    cout << "Postfix expression: " <<obj.infixToPrefix(infix)<< endl;
    return 0;
}
